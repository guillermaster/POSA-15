...
public class MainActivity extends LifecycleLoggingActivity {
	...
	public void downloadImage(View view) {
		try {
			...
			Intent getDownloadImageIntent = makeDownloadImageIntent(getUrl());
			...
			// method.
			getDownloadImageIntent.putExtra(TAG, "download");
			startActivityForResult(getDownloadImageIntent,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			...
			if (requestCode == DOWNLOAD_IMAGE_REQUEST) {
				...
				// file.
				Intent launchGalleryIntent = makeGalleryIntent(data.getData()
						.toString());

				// Start the Gallery Activity.
				if (launchGalleryIntent != null)
					startActivity(launchGalleryIntent);
				else {

					Toast galleryToast = Toast
		...
		// download contents at the given URL.
			...
			Toast resultToast = Toast.makeText(getBaseContext(),
					"Reslut code wasn't successful", Toast.LENGTH_SHORT);
			resultToast.show();
		}
	}
	...
	protected Uri getUrl() {
		...
		// Reference :
		// http://stackoverflow.com/questions/4905075/how-to-check-if-url-is-valid-in-android?lq=1
		Pattern URL_PATTERN = Patterns.WEB_URL;
		boolean isURL = URL_PATTERN.matcher(uri).matches();
